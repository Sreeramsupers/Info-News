...
export default class GetSources extends Component {
	state = {
		news: [],
	};
	...
	componentDidMount() {
		this.props.navigation.setOptions({
			title: this.props.route.params.source.name,
		});

		fetch(`${BASE_URL}/everything/${this.props.route.params.source.id}.json`)
			.then((res) => res.json())
			.then((response) => {
				this.setState({
	...
	render() {
		...
		return (
			<View>
				<Header
					headText={this.props.route.params.source.name}
